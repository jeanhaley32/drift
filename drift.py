...
class SignalScene(Scene):
    ...
    def update(self, dt, frame, st):
        cy, cx, pts = self._nodes(st.get("conns", 0) or 3)
        # latency pulse rings
        lat = st.get("latency") or 80
        period = clamp(lat / 200.0, 0.2, 1.3) * 2.0
        self.pulse_t += dt
        if self.pulse_t >= period:
            self.pulse_t = 0.0
            self.pulse.append(0.0)
        self.pulse = [r + dt * 16 for r in self.pulse if r < max(self.h, self.w)]
        # spawn packets along links, rate scales with throughput
        if pts and random.random() < (0.1 + st["net"] * 0.9) * dt * REF_FPS:
            i = random.randrange(len(pts))
            outbound = random.random() < (st["up_kbps"] / (st["up_kbps"] + st["down_kbps"] + 1))
            self.packets.append([i, 0.0, 0.9 + st["net"] * 2.0, outbound])
        for p in self.packets:
            p[1] += dt * p[2]
        self.packets = [p for p in self.packets if p[1] < 1.0]
        self._cache = (cy, cx, pts)
    ...
    def draw(self, scr, frame, st):
        ...
        # links
        for (py, px) in pts:
            plot_line(scr, cy, cx, py, px, "·", cp(C_GREEN) | curses.A_DIM)
        # latency pulse
        import math
        for r in self.pulse:
        ...
        for (i, t, sp, outb) in self.packets:
            if i >= len(pts):
                continue
            py, px = pts[i]
            if outb:
                yy = lerp(cy, py, t); xx = lerp(cx, px, t)
                ch, col = ">", C_YELLOW
            else:
                yy = lerp(py, cy, t); xx = lerp(px, cx, t)
                ch, col = "<", C_GREEN
            putch(scr, yy, xx, ch, cp(col) | curses.A_BOLD)
        # endpoint nodes (LAN device count tints how many are "lit")
        for k, (py, px) in enumerate(pts):
            putch(scr, py, px, "O", cp(C_WHITE) | curses.A_BOLD)
        # central YOU node
        label = f"[ {self.tele.host} ]"
        put(scr, cy, cx - len(label) // 2, label, cp(C_MAGENTA) | curses.A_BOLD | curses.A_REVERSE)
        put(scr, cy + 1, cx - len(self.tele.d['lan_ip']) // 2, self.tele.d["lan_ip"],
            cp(C_CYAN) | curses.A_DIM)
        # status line
        ssid = st.get("ssid") or "—"
        vpn = "  VPN" if st.get("vpn") else ""
        loss = "  !LOSS" if st.get("loss") else ""
        rem = f"  →{st['remotes']} hosts" if st.get("remotes") else ""
        put(scr, self.h - 2, 3,
            f"SSID {ssid[:18]}   conns {st.get('conns',0)}   LAN {st.get('lan',0)} dev{vpn}{loss}{rem}",
            cp(C_GREEN) | curses.A_BOLD)
        if st.get("pubip"):
            put(scr, self.h - 3, 3, f"public {st['pubip']}", cp(C_YELLOW) | curses.A_DIM)

    def hud(self, st):
        return [("nodes = connections", f"{st.get('conns',0)}"),
                ("packets = throughput", f"↓{st['down_kbps']:,.0f} ↑{st['up_kbps']:,.0f} KB/s"),
                ("pulse = latency", f"{st['latency']:.0f} ms" if st['latency'] else "—"),
                ("LAN devices", f"{st.get('lan',0)}"),
                ("nearby wifi", f"{len(st.get('neighbors') or [])}")]
